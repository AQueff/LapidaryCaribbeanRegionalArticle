...
library(knitr)
library(kableExtra)
periodization=read.csv(file = "Table_periodization.csv",na.strings = "0",encoding="UTF8")
...
library(knitr)
library(kableExtra)
dataset_rawmaterials=read.csv(file = "Table_rawmaterials.csv",na.strings = "0",encoding = "UTF-8")
...
library(knitr)
library(kableExtra)
dataset_types=read.csv(file = "Table_types.csv",na.strings = "",encoding = "UTF-8")
...
# Data loading
df_rawmaterial <- read.csv(file = "bdd_paaf_rawmaterial.csv", row.names = 1, check.names = F)
...
####################          WARNING      ########################
########### Knitr 1.43 has issue with all heatmap chunks   ########
###########       Need to stay on Knitr 1.42      #################
#######  (may be OK later, don't know when they fix it) ###########
###################################################################
...
df_typerawsites$Period <- factor(df_typerawsites$Period, levels=c('Early Ceramic', 'Middle Ceramic', 'Late Ceramic', 'Late-Final Ceramic'))
...
# Giving the edges the starting and ending points which are the coordinates of the sites.
...
ggraph(lay) + country_shapes + mapcoords + maptheme +
  ...
  scale_edge_colour_gradient(low = "#fafafa",high = "#008080") +
  geom_node_point(aes(color=Paaf_bdd$Color), shape = 19,size = 3,stroke = 0.5) +
    scale_color_identity("Legend", labels = levels(as.factor(Paaf_bdd$Period)),guide = "legend") +
  geom_node_text(aes(label = name), repel = TRUE, size = 3,
                 color = "black", fontface = "bold")
